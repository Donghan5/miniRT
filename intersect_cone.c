...
/*   Updated: 2024/12/12 19:14:14 by pzinurov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	validate_t(double t, t_ray ray, t_cone *co)
{
	...
	center = co->coordinates;
	half_height = co->co_height / 2.0;
	if (t < EPSILON)
		return (-1.0);
	if (fabs(vec3_dot(vec3_sub(vec3_add(ray.origin, vec3_mul(ray.direction,
							t)), center), axis)) > half_height)
		return (-1.0);
	return (t);
}
