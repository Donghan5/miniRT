...
/*   Updated: 2024/12/10 14:31:33 by pzinurov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	add_diffuse_spec(t_hit_material closest, t_info *info, t_vec3 normal, t_ray ray, t_color *total_diffuse, t_color *total_spec)
{
	int	i;
	...
	while (i < info->scene.light_n)
	{
		...
		double shadow = check_shadow(info, vec3_add(closest.hit_point, vec3_mul(normal, EPSILON)), light->coordinates);

		double diffuse = fmax(0.0, vec3_dot(normal, light_dir)) * closest.diffuse_strength * light->l_brightness * shadow;
...
*/
t_color	add_lights(t_hit_material closest, t_info *info, t_vec3 normal, t_ray ray)
{
	t_color	result;
	...
	add_diffuse_spec(closest, info, normal, ray,
